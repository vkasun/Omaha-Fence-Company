...
@$link = mysql_connect( '192.168.2.175', 'fw1421057947', 'b13KxDuk5d3fyivE1onv1RuIQt3130' );
if ( ! $link ) {
	die( sprintf( "Could not connect to the MySQL server: %s\n", mysql_error() ) );
}
echo "Connected successfully to the MySQL server\n";

@$db_selected = mysql_select_db( db1891668313_staging, $link );
if ( ! $db_selected ) {
	die ( sprintf( "Could not select the database '%s': %s\n", 'db1891668313_staging',  mysql_error() ) );
}
echo "Database selected successfully\n";

echo "\nChecking tables for errors:\n";
$tables = mysql_query( "SHOW TABLES LIKE 'wp_777y19fi0b_%'" );
