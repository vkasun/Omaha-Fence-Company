...
function google_business_reviews_rating_admin(popstate) {
	...
	if (jQuery('#google-business-reviews-rating-settings').length && jQuery('.section', '#wpbody-content').length) {
		...
		jQuery('#review-limit, #review-limit-hide, #review-limit-set, #review-limit-all', '#wpbody-content').on('change', function() {
			...
			jQuery(':input', jQuery('.show-reviews', '#wpbody-content')).each(function() {
                if (!jQuery(this).is(':disabled') && (typeof jQuery('#review-limit').val() == 'string' && jQuery('#review-limit').val() == '0' || typeof jQuery('#review-limit').val() == 'number' && jQuery('#review-limit').val() <= 0)) {
					...
					jQuery(this).removeProp('disabled').removeAttr('disabled');
				}
            });
		});
		...
		jQuery('#stylesheet-none, #stylesheet-compressed, #stylesheet-standard', '#wpbody-content').on('change', function() {
			...
			else if (jQuery('#stylesheet-none').is(':checked') && jQuery('#reviews-theme', '#wpbody-content').is(':disabled')) {
				jQuery('#reviews-theme', '#wpbody-content').removeProp('disabled').removeAttr('disabled');
			}
		});
	...
	jQuery('#import-button').on('click', function() {
		...
		jQuery.post(google_business_reviews_rating_admin_ajax.url, data, function(response) {
			if (response.success) {
				...
				jQuery('#import-process-button, #html-import, #advanced > .introduction, #google-business-reviews-rating-settings-styles-scripts, #google-business-reviews-rating-settings-cache, #google-business-reviews-rating-settings-reset').show();
	...
	if (jQuery('#html-import').length) {
		jQuery('#import-clear-button').on('click', function(event) {
			...
			jQuery('#advanced').removeClass('import-results');
			jQuery('#import-button, #import-clear-button').hide();
			jQuery('#import-process-button, #html-import, #html-import-review-text, #html-import-existing-label, #advanced > .introduction, #google-business-reviews-rating-settings-styles-scripts, #google-business-reviews-rating-settings-cache, #google-business-reviews-rating-settings-reset').show();
			jQuery('#html-import').val('');
			reviews = [];
		});
		...
		jQuery('#html-import, #import-process-button').on('change blur click', function(event) {
			...
			if (e != null) {
				...
				jQuery('#import-process-button, #html-import, #html-import-review-text, #html-import-existing-label, #advanced > .introduction, #google-business-reviews-rating-settings-styles-scripts, #google-business-reviews-rating-settings-cache, #google-business-reviews-rating-settings-reset').hide();
				jQuery('#html-import').addClass('valid');
