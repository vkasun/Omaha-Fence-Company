...
class google_business_reviews_rating
{
	...
	public function admin_init()
	{
		...
		register_setting(__CLASS__ . '_settings', __CLASS__ . '_color_scheme', array('type' => 'string'));
	...
	public function sync()
	{
		...
		{
		case 168:
			if (!empty($last_retrieval) && isset($last_retrieval['place_id']) && $last_retrieval['place_id'] == $this->place_id && isset($last_retrieval['time']) && ((defined('DISABLE_WP_CRON') && DISABLE_WP_CRON && (time() - $last_retrieval['time']) < 14514900) || ((!defined('DISABLE_WP_CRON') || defined('DISABLE_WP_CRON') && !DISABLE_WP_CRON) && (time() - $last_retrieval['time']) < 14514300)))
			{
				return FALSE;
			}
			...
			break;
		case 24:
			if (!empty($last_retrieval) && isset($last_retrieval['place_id']) && $last_retrieval['place_id'] == $this->place_id && isset($last_retrieval['time']) && ((defined('DISABLE_WP_CRON') && DISABLE_WP_CRON && (time() - $last_retrieval['time']) < 86100) || ((!defined('DISABLE_WP_CRON') || defined('DISABLE_WP_CRON') && !DISABLE_WP_CRON) && (time() - $last_retrieval['time']) < 72000)))
			{
				return FALSE;
			}
			...
			break;
		case 6:
			if (!empty($last_retrieval) && isset($last_retrieval['place_id']) && $last_retrieval['place_id'] == $this->place_id && isset($last_retrieval['time']) && ((defined('DISABLE_WP_CRON') && DISABLE_WP_CRON && (time() - $last_retrieval['time']) < 21300) || ((!defined('DISABLE_WP_CRON') || defined('DISABLE_WP_CRON') && !DISABLE_WP_CRON) && (time() - $last_retrieval['time']) < 19800)))
			{
				return FALSE;
			}
...
';
	...
	public function admin_ajax()
	{
		...
		$section = (isset($_POST['section']) && is_string($_POST['section']) && !preg_match('/^setup$/i', $_POST['section'])) ? preg_replace('/[^\w_-]/', '', strtolower(sanitize_text_field($_POST['section']))) : NULL;
		$review = (isset($_POST['review']) && is_string($_POST['review'])) ? preg_replace('/[^\w_]/', '', wp_kses_stripslashes(sanitize_text_field($_POST['review']))) : NULL;
		...
		$sort = (isset($_POST['sort']) && is_string($_POST['sort']) && preg_match('/^reviews$/i', $section) && !preg_match('/^relevance(?:[_-]desc)?$/', $_POST['sort'])) ? preg_replace('/[^\w_-]/', '', strtolower(sanitize_text_field($_POST['sort']))) : NULL;
		$submitted = (isset($_POST['submitted']) && is_string($_POST['submitted']) && is_string($_POST['submitted'])) ? wp_kses_stripslashes(sanitize_text_field($_POST['submitted'])) : NULL;
		$status = (isset($_POST['status']) && (is_bool($_POST['status']) && $_POST['status'] || is_string($_POST['status']) && preg_match('/^true$/i', $_POST['status'])));
		$api_key = (isset($_POST['api_key']) && is_string($_POST['api_key']) && strlen($_POST['api_key']) >= 10 && strlen($_POST['api_key']) <= 255) ? wp_kses_stripslashes(sanitize_text_field($_POST['api_key'])) : NULL;
		$place_id = (isset($_POST['place_id']) && is_string($_POST['place_id']) && strlen($_POST['place_id']) >= 10 && strlen($_POST['place_id']) <= 255) ? wp_kses_stripslashes(sanitize_text_field($_POST['place_id'])) : NULL;
		$language = (isset($_POST['language']) && is_string($_POST['language']) && strlen($_POST['language']) >= 2) ? wp_kses_stripslashes(sanitize_text_field($_POST['language'])) : NULL;
		$update = (isset($_POST['update']) && is_numeric($_POST['update'])) ? intval($_POST['update']) : NULL;
		$ids = (isset($_POST['id']) && is_string($_POST['id'])) ? preg_split('/,\s*/', wp_kses_stripslashes(sanitize_text_field($_POST['id']))) : array();
		...
		$custom_styles = (isset($_POST['custom_styles']) && is_string($_POST['custom_styles']) && mb_strlen($_POST['custom_styles']) > 2 && !preg_match('/<\?(?:php|=)/i', $_POST['custom_styles'])) ? wp_kses_stripslashes(sanitize_text_field($_POST['custom_styles'])) : NULL;
		$import_type = (isset($_POST['import_type']) && is_string($_POST['import_type']) && preg_match('/^(?:original|translation)$/i', $_POST['import_type'])) ? wp_kses_stripslashes(sanitize_text_field($_POST['import_type'])) : NULL;
		...
		case 'structured_data':
			...
			{
				$data['telephone'] = wp_kses_stripslashes(sanitize_text_field($_POST['telephone']));
			}
			
			if (isset($_POST['business_type']) && is_string($_POST['business_type']) && preg_match('/^[\w\s_-]{1,64}$/i', $_POST['business_type']))
			{
				$data['business_type'] = wp_kses_stripslashes(sanitize_text_field($_POST['business_type']));
			}
...
';
	...
	private function admin_preview($post = array())
	{
		...
		if (!is_array($post) || is_array($post) && empty($post))
		{
			...
			{
				if (!is_string($k) && !is_numeric($k) && !is_bool($v) && !is_string($v) && !is_numeric($v))
				{
					continue;
				}

				$post[sanitize_key($k)] = wp_kses_stripslashes(sanitize_text_field($v));
			}
		}
	...
	public function server_ip()
	{
		...
		if (function_exists('gethostname') && function_exists('gethostbyname'))
		{
			...
			{
				wp_cache_set('server_ip', strtolower($string), __CLASS__, HOUR_IN_SECONDS);
				return $string;
			}
		}
		...
		{
			wp_cache_set('server_ip', $_SERVER['SERVER_ADDR'], __CLASS__, HOUR_IN_SECONDS);
	...
	private function reviews_filter($filters = NULL, $atts = NULL)
	{
		...
		$count = 0;
		$ids = (array_key_exists('id', $filters) && is_numeric($filters['id']) && $filters['id'] > 0) ? array(intval($filters['id'])) : ((array_key_exists('id', $filters) && preg_match('/^(?:\d+)(?:,\s*(?:\d+))+$/', $filters['id'])) ? array_unique(preg_split('/[^\d]+/', $filters['id'])) : array());
		...
		$review_text_inc = (array_key_exists('review_text_inc', $filters) && strlen($filters['review_text_inc']) > 1) ? array_unique(preg_split('/,\s*/', $filters['review_text_inc'], 10)) : array();
		$review_text_exc = (array_key_exists('review_text_exc', $filters) && strlen($filters['review_text_exc']) > 1) ? array_unique(preg_split('/,\s*/', $filters['review_text_exc'], 10)) : array();
...
';
		...
		case 'html':
			...
			$html .= '<table id="reviews-table" class="wp-list-table widefat fixed striped reviews-table' . (($show_place_id) ? ' places' : '') . '" data-languages="' . esc_attr(json_encode($this->languages)) . '">
    <thead>
        <tr>
...
';
	...
	public function wp_display($atts = NULL, $content = NULL, $shortcode = NULL)
	{
		...
		if ($type_check == NULL && preg_match('/^.+_links?$/i', $shortcode))
		{
		...
		case 'google_reviews':
			...
			if (is_bool($link) && !$link && is_numeric($limit) && $limit == 0 && preg_match('/\b(?:tiny|badge)\b/', $theme))
			{
				$link = $reviews_url;
			}
			elseif ((is_bool($link) && $link || is_string($link)) && (!is_numeric($limit) || is_numeric($limit) && $limit > 0))
			{
				$link = (preg_match('/\b(?:tiny|badge)\b/', $theme)) ? $reviews_url : FALSE;
			}
			...
			if (!array_key_exists('summary', $atts) && !array_key_exists('icon', $atts) && !array_key_exists('name', $atts) && !array_key_exists('vicinity', $atts) && preg_match('/\b(?:tiny\b.*badge|badge\b.*tiny)\b/', $theme))
			{
				$icon = FALSE;
				$name = FALSE;
				$vicinity = FALSE;
...
';
				...
				if ((is_bool($attribution) && $attribution || is_string($attribution) && strlen($attribution) >= 1) && preg_match('/\btiny\b/', $theme))
				{
...
';
				...
				elseif (!is_numeric($limit) || is_numeric($limit) && $limit > 0)
				{
					...
					elseif (!is_array($review_item_order))
					{
						...
						elseif (preg_match('/\bbubble\b/', $theme) && preg_match('/\bcenter\b/', $theme))
						{
							if ($options['review_item_author_switch'])
							{
								$options['review_item_text_first'] = TRUE;
					...
					$options['avatar'] = ($avatar && (is_string($review_item_order) || is_array($review_item_order) && in_array('avatar', $review_item_order)));
...
';
	...
	private function review_item($data = NULL, $options = NULL, $type = 'all')
	{
		...
		case 'all':
			break;
		case 'review':
			if (isset($review_text) && !$review_text || mb_strlen($data['text']) == 0)
			{
				return $html;
			}
