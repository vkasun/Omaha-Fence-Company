...
class Amfence_Locations_Settings {
	...
	function amfence_locations_add_menu( $menu ) {
		...
		$menu['page_title'] = __( 'AMFence Locations Settings', 'amfence-locations' );  // The page title. Defaults to 'Section214 Settings'.
		...
		$plugin_settings = array(
			'general' => array(
				'main' => array(
					...
					array(
						'id'   => 'refresh_cache_field',
						'name' => __( 'Refresh Cache', 'amfence-locations' ),
						//'desc' => __( '<br /><strong><em>Only Required if Slave is selected above ^</em></strong><br />The website URL of the master site', 'amfence-locations' ),
						'type' => 'hook',
					)
				)
	...
	function amfence_locations_refresh_cache_field( $tabs ) {
		$request = $_REQUEST;
		$server = $_SERVER;
		$button_url = $_SERVER['REQUEST_URI'].'&refreshCache=true';
		if(isset($_REQUEST['refreshCache']) && $_REQUEST['refreshCache']){
			$button_url = $_SERVER['DOCUMENT_URI'].'?page=amfence-locations-settings&refreshCache=true';
			$locations_obj = new AMFence_Locations();
			$locations_obj->refresh_locations();
		}

		echo '<a href="'.$button_url.'"><div class="button button-primary">Refresh Locations Cache</div></a>';
		if(isset($_REQUEST['refreshCache']) && $_REQUEST['refreshCache']){
			echo '<br />Location cache has been refreshed';
		}
	}
